...
import run=require("../../model/runModel")
...
function redirect(res,bHttps,opt,location,resolve,reject) {
    ...
    var req3=request1(opt1,function (res3) {
        if(res3.statusCode==302)
        {
            ...
            redirect(res,bHttps,opt,res3.headers.location,resolve,reject)
        }
        else
        {
...
var onProxy = function (ctx) {
    return new Promise(function (resolve,reject) {
        ...
        run.create({
            user:req.headers["userModel-doclever"],
            host:opt.host,
            path:opt.path,
        },function (err) {
        ...
        var req2 = request(opt, function (res2) {
            if(res2.statusCode==302)
            {
                ...
                redirect(res,bHttps,opt,res2.headers.location,resolve,reject)
            }
            else
            {
...
router.all("/proxy",async function (ctx) {
    await onProxy(ctx)
})
