 ...
 */
import error=require("./error.json");
...
import {IError_info, IKoa_ctx} from "../types/global";
var fsAsync=blue.promisifyAll(fs);
var child_process=blue.promisifyAll(import("child_process"));
require("./Base64")
var testModel=null;
...
export function err(ctx,code,msg) {
    ctx.body={
        code:code,
        msg:msg
    }
...
export function ok(ctx,data,msg?) {
    if(!msg)
    {
        msg="ok";
    }
...
function tr(type:IError_info) {
    throw type;
}
export {tr as throw}

export function stopThen() {
    var err:any={};
    err.stop=true;
    throw err;
}
...
function ch(ctx,e) {
    ...
    {
        err(ctx,e.type,e.msg);
    }
    else
    {
        console.log(e);
        err(ctx,error.systemReason,"服务器错误");
    }
}
...
export function dateDiff(startTime, endTime, diffType) {
    ...
    return Math.abs(parseInt(String((eTime.getTime() - sTime.getTime()) / parseInt(String(divNum)))));
}
...
export function validateArgv(count) {
...
export function validateParam(val,validate) {
...
export function  delImg(filePath) {
    if(filePath)
    {
        fs.access(con.filePath+filePath.replace(/\//g,path.sep),fs.constants.F_OK,function (err) {
            if(!err)
            {
                fs.unlink(con.filePath+filePath,function () {
...
export async function getFileSize(filePath) {
    ...
    if(bExist)
    {
        let obj=await fsAsync.stat(con.filePath+filePath.replace(/\//g,path.sep));
        if(obj)
        {
            size+=obj.size;
        }
...
export function convertToJSON(data,obj,info?) {
    var mock=function (data) {
        ...
        else
        {
            ...
            else if(str.startsWith("in"))
            {
                ...
                {
                    return parseFloat(String(temp));
                }
                else if(data.type==2)
                {
                    return Boolean(eval(String(temp)));
                }
            }
...
export function mock(data,info) {
    if(!data || data.trim()[0]!="@")
    {
        if(data)
        {
...
export function handleMockInfo(param,query,body,header,objInterface,url) {
    var info={
        param:param,
        query:query,
        header:header,
...
export function inArrKey(str,arr,key) {
...
var encrypt=function (type,val,salt) {
    ...
    var arrFunc=[(<any>global).BASE64.encoder,CryptoJS.MD5,CryptoJS.SHA1,CryptoJS.SHA256,CryptoJS.SHA512,CryptoJS.SHA3,CryptoJS.RIPEMD160]
...
var runTest=async function (obj,global,test,root,opt) {
    ...
    if(method=="POST" || method=="PUT" || method=="PATCH")
    {
        ...
        else
        {
            ...
            else if(obj.bodyInfo.rawType==2)
            {
                ...
                convertToJSON(result,obj1,null);
                body=obj1;
            }
            else
            {
    ...
    return func.then(function (result) {
        ...
        res.second=((Date.now()-startDate.getTime())/1000).toFixed(3);
        ...
        root.output+="["+moment().format("YYYY-MM-DD HH:mm:ss")+"]结束运行接口："+obj.name+"(耗时：<span style='color: green'>"+((Date.now()-startDate.getTime())/1000).toFixed(3)+"秒</span>)<br>"
        return {
            status:200,
            header:{},
            data:err
...
export async function runTestCode(code,test,global1,opt,root,argv,mode,__id,level) {
    if(!testModel)
    {
    ...
    for(var i=0;i<arr.length;i++)
    {
        ...
        if(type=="1")
        {
            ...
            if(!bFind)
            {
                objInfo=await projectModel.findOne({
                    _id:query.project
                ...
                if(query.version)
                {
                    let objVersion=await versionModel.findOne({
                        _id:query.version
                    },"baseUrls after before");
                    if(!objVersion)
                    {
            ...
            if(o.example)
            {
                try
                {
                    let objExample=await exampleModel.findOne({
                        _id:o.example
                    },"param");
        ...
        else if(type=="2")
        {
            ...
            testObj=await testModel.findOne(obj.length==24?{
                _id:obj
            }:{
                id:obj
            },"-output",{
            ...
            testObj=await testModel.populate(testObj,{
                path:"group",
                select:"name"
            })
            var code;
            ...
            text="(function () {var argv=Array.prototype.slice.call(arguments);return runTestCode('"+code+"',"+JSON.stringify(testObj)+",global1,opt,root,argv,'"+testMode+"',"+(level==0?objId:undefined)+","+(level+1)+")})"
        }
        else
        {
...
export function convertToCode(data) {
    var str="";
...
export function sendMail(smtp,port,user,pass,to,subject,content) {
...
export function sendSMS(method,baseUrl,param) {
...
export async function init() {
    let setConfig=async function () {
        if((!fs.existsSync(path.join(__dirname, "../../config.json")) || !require("../../config.json").db) && process.argv.length<=2)
        {
            ...
            let question=function (title):Promise<string>
            {
                return new Promise(function (resolve,reject) {
                    read.question(title,function (answer:string) {
                        resolve(answer);
                    })
                })
            }
            ...
            {
                let val:string=await question(arr[i]);
                if(!val)
                {
                    i--;
                    continue;
        ...
        if(argv.port)
        {
            con.port=parseInt(String(argv.port));
        }
    }
    let patch=async function () {
        ...
        files=files.filter(function (obj) {
            if(obj!="." && obj!=".." && (obj.endsWith(".js") || obj.endsWith(".ts")))
            {
                return true;
            }
            else
            ...
            return versionDiff(obj1,obj2)?1:0;
        })
...
export async function getMockParam(clientParam,obj,type,version) {
    var arr=[];
    if(!statusModel)
    {
        ...
        for(let key in  clientParam)
        {
            ...
            if(objParam)
            {
                ...
                else
                {
                    ...
                    let objStatus=await status.findOne(query)
                    if(objStatus)
                    {
                        let bFind=false;
...
export function handleResultData(name,data,result,originObj,show,input?,bArr?) {
...
export async function runPoll(arr) {
...
export function handleGlobalVar(str,global) {
    var type;
...
export function getPostmanGlobalVar(str,baseUrls) {
    let arr=str.match(/\{\{(.+?)\}\}/g);
    if(!arr)
    {
        return;
...
export function getNowFormatDate(fmt,date) {
    date=date || new Date();
    var o = {
...
export async function createStatistic() {
    ...
    obj.interface=await inter.count({
        createdAt:{
            $gte:date
        }
    })
    obj.project=await project.count({
        createdAt:{
            $gte:date
        }
    })
    obj.team=await team.count({
        createdAt:{
            $gte:date
        }
    })
    obj.user=await user.count()
    obj.userRegister=await user.count({
        createdAt:{
            $gte:date
        }
    })
    obj.userLogin=await user.count({
        lastLoginDate:{
            $gte:date
        }
    })
    await statistic.create(obj);
}
...
export async function backup(db,version) {
